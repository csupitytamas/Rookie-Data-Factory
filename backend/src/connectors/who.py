...
class WHOConnector(BaseConnector):
    def __init__(self, **kwargs):
        ...
        try:
            ...
            logger.warning(f"Error loading valid entities: {e}")

    # Az elérhető entitások listájának lekérése a metaadatokból
    def _load_valid_entities(self) -> List[str]:
        response = self.make_request("$metadata")
        xml = response.text
        return re.findall(r'<EntitySet Name="([^"]+)"', xml)

    # Meghatározza, hogy az indikátorhoz létezik-e közvetlen entitás, vagy a Fact táblát kell használni
    def resolve_entity(self, indicator: str) -> str:
        if indicator in self.valid_entities:
            return indicator
        # Ha nincs specifikus entitás, a központi Fact táblában keresünk
        return "Fact"

    # Indikátorok listájának lekérése a felhasználói felület számára
    ...
    def parse_response(self, response):
        ...
        for row in rows:
            ...
            # Mezők normalizálása és átnevezése az egységes formátum érdekében
