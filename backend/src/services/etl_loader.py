...
def load_to_target_table(pipeline_id: int, db: Session):
    ...
    try:
        ...
        db.commit()
        logger.info(
            f"{len(insert_data)} load to {pipeline.target_table_name}"
        )
...
def _load_legacy_method(pipeline: ETLConfig, db: Session):
    ...
    db.execute(stmt)
    db.commit()
    logger.info(f"{len(insert_data)} load to {pipeline.target_table_name}")